...
function addTextAreaBtn(method, _id="") {
    ...
    btn.addEventListener("click", async function () {
        ...
        //se c'è un id (cioè se la richiesta PATCH O PUT)
        if(_id)
            resource += "/" + _id
...
btnDelete.addEventListener("click", async function () {
    ...
    if (confirm("Vuoi veramente cancellare il record" + JSON.stringify(filters) + "?")) {
        let resource = `/${currentCollection}/${filters}`;
...
async function getData(filters = {}) {
    ...
    if (HttpResponse.status == 200) {
        ...
        for (let item of HttpResponse.data) {
            ...
            div.addEventListener("click", patchCurrent())
            td.append(div);
            // put
...
async function patchCurrent(_id) {
    ...
    if (HttpResponse.status == 200) {
        ...
        // rimuovere la chiave da un json
        delete(current._id)
        const textArea = document.createElement("textarea");
        divDettagli.append(textArea)
        textArea.value = JSON.stringify(current, null, 3)
