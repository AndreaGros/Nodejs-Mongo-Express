...
// o file JS possono essere importanti
const inviaRichiesta = require("./libreria.js")
...
app.post("/api/people", async (req, res, next) => {
    const _URL = "https://randomuser.me/api?result=5"
    const response = await inviaRichiesta("GET", _URL)

    if (response.status == 200) {
        res.send(response.data)
    }
    else
        res.status(response.status).send("Remote server error: " + response.err)
})
